...
- **成分での確認** (`lib/ingredients.ts`): バーコード登録した手持ちは、
  Open Beauty Facts から**全成分表示**も取得して保存する。手順が挙げる「注目成分」が
  実際に入っているかを照合し、各工程に `✓ ヒアルロン酸` / `セラミド は入っていません`
  のように表示する。
  - 手順側は「ヒアルロン酸」のような総称、実際の表示は「ヒアルロン酸Na」「加水分解
    ヒアルロン酸」のような個別名なので、**成分ファミリー単位**の正規表現で照合する。
  - 「トーンアップ成分」「皮脂吸着パウダー」など**機能の呼び名は判定対象外**にする
    （成分名として照合できないため、誤った×を出さない）。
  - 成分データが無い手持ちには**そもそも成分の話を出さない**（`unknown` と `absent` を区別）。

  > ⚠️ 全成分表示から分かるのは**「入っているか」だけ**です。配合量は公開されず
  > （1%以下は順不同で並ぶ規則のため順序からも特定できない）、処方全体で効き方も変わります。
  > そのため **「同等の効果がある」「成分的に代替できる」とは断定しません**。
  > 色味判定と同じく、断定しないことを優先しています。
